...
let data = {
    ...
    const statuslist = await axios.post(`${baseUrl}/api/forecaststatus/show`,data);
  
  
    var dataSet;
    if (
        statuslist.status === 200 &&
        statuslist.data.status === 200
    ) {
        
      let list = [...statuslist.data.getList];
      let listarr = list.map((item, index, arr) => {
        let editbtn = !!(permission?.["BusinessForecastStatus"]?.can_edit) ? '<i class="fas fa-edit text-info mx-2 h6" style="cursor:pointer" title="Edit"></i> ' : '';
           
      let  deletebtn =  !!(permission?.['BusinessForecastStatus']?.can_delete) ? '<i class="fas fa-trash-alt text-danger h6  mx-2" style="cursor:pointer"  title="Delete" data-action="DELETE"></i>' : '';
    ...
    $("#dataTable tbody").on("click", "tr .fa-edit", function () {
      ...
      navigate(`/tender/master/businessforecaststatus/edit/${rowdata.id}`);
    });

    $("#dataTable tbody").on("click", "tr .fa-trash-alt", function () {
      let rowdata = table.row($(this).closest("tr")).data();
const data ={
del_id : rowdata.id,
token : localStorage.getItem('token')
         ...
         axios.post(`${baseUrl}/api/forecaststatus/destroy`,data).then((res) => {
      if (res.status === 200) {
