...
const DistrictMasterList = () => {
  ...
  const deleterecord = async (id) => {
    let response =  axios.delete(`${baseUrl}/api/district/${id}`)
    return response;
  }

  const getList = async () => {
    const districtlist = await axios.get(`${baseUrl}/api/district`);
    ...
    var dataSet;
    if (
      districtlist.status === 200 &&
      districtlist.data.status === 200
    ) {
      let list = [...districtlist.data.district];
      let listarr = list.map((item, index, arr) => {
        let editbtn = !!(permission?.["Districts"]?.can_edit) ? '<i class="fas fa-edit text-info mx-2 h6" style="cursor:pointer" title="Edit"></i> ' : '';
            let deletebtn =  !!(permission?.["Districts"]?.can_delete)  ?  '<i class="fas fa-trash-alt text-danger h6  mx-2" style="cursor:pointer"  title="Delete"></i>' : '';
    ...
    $("#dataTable tbody").on("click", "tr .fa-edit", function () {
      ...
      navigate(
        `/tender/master/districtmaster/districtcreation/${rowdata.id}`
      );
    });
    ...
    $("#dataTable tbody").on("click", "tr .fa-trash-alt", async function () {
      ...
      Swal.fire({
        text: `Are You sure, to delete ${rowdata.district_name}?`,
        icon: "warning",
        ...
        if (willDelete.isConfirmed) {
         ...
         if (response.data.status === 200) {
            Swal.fire({ //success msg
              icon: "success",
              title: `${rowdata.district_name} `,
              text: `${rowdata.district_name} has been removed!`,
              timer: 1500,
