...
const CallTypeList = () => {
  ...
  const deleterecord = async (id) => {
    let response =  axios.delete(`${baseUrl}/api/calltype/${id}`)
    return response;
  }

  const getList = async () => {
    const calltypelist = await axios.get(`${baseUrl}/api/calltype`);
    ...
    var dataSet;
    if (
      calltypelist.status === 200 &&
      calltypelist.data.status === 200
    ) {
    ...
    $("#dataTable tbody").on("click", "tr .fa-edit", function () {
      ...
      navigate(
        `/tender/master/calltypemaster/edit/${rowdata.id}`
      );
    });
